...
// ----------------------------------------------------------------------------
void PhysicsEngine::initializeWorld_Classic_Cellular_Langrand_Hexagonal_Perfect(void)
{
...
//		pInconel->i_MaterialType = __ELASTIC;
		...
		pInconel->d_YieldStress = 250.0e6;
		pInconel->d_Hardening_Isotropic_C0 = 15.0;
		pInconel->d_Hardening_Isotropic_C1 = 450.0e6;
	}
	...
	glm::dvec3 d3Center_Array			= glm::dvec3(0.5*dDiameter_Outer, 0.0, 0.5*d3_Length_Grid.z);
	glm::dvec3 d3Center_Platen_Top		= glm::dvec3(1.0*d3_Length_Cell.x+0.5*d3Dimension_Platen_Top.x, -0.0005+d3Center_Array.y + (i2Array_Count.y-1.0)*d2Array_Offset.y+0.5*dDiameter_Outer+0.5*d3Dimension_Platen_Top.y + 1.0*d3_Length_Cell.y, 0.5*d3_Length_Grid.z);

//	glm::dvec3 d3Dimension_Platen_Bottom	= glm::dvec3(i2Array_Count.x*dDiameter_Outer+0.001, 4.0*d3_Length_Cell.y, dOffset);
//	glm::dvec3 d3Dimension_Platen_Top		= glm::dvec3(i2Array_Count.x*dDiameter_Outer+0.001, 2.0*d3_Length_Cell.y, dOffset);
//
//	glm::dvec3 d3Center_Platen_Bottom	= glm::dvec3(0.5*d3_Length_Grid.x, 0.5*d3Dimension_Platen_Bottom.y + 3.0*d3_Length_Cell.y, 0.5*d3_Length_Grid.z);
//	glm::dvec3 d3Center_Array			= glm::dvec3(0.5*d3_Length_Grid.x - 3.5*dDiameter_Outer, d3Center_Platen_Bottom.y + 0.5*d3Dimension_Platen_Bottom.y + 0.5*dDiameter_Outer, 0.5*d3_Length_Grid.z);
//	glm::dvec3 d3Center_Platen_Top		= glm::dvec3(0.5*d3_Length_Grid.x, d3Center_Array.y + (i2Array_Count.y-1.0)*d2Array_Offset.y+0.5*dDiameter_Outer+0.5*d3Dimension_Platen_Top.y + 3.0*d3_Length_Cell.y, 0.5*d3_Length_Grid.z);
	...
	{// sample
		...
		{// sample array
			...
			{
				glm::dvec3 d3Center_Ring = glm::dvec3(0.0,0.0,0.0);

				if(iy % 2 == 0)
					d3Center_Ring = d3Center_Array + glm::dvec3(ix*d2Array_Offset.x, iy*d2Array_Offset.y,0.0);
				else if(iy % 2 == 1)
					d3Center_Ring = d3Center_Array - glm::dvec3(0.5*dDiameter_Outer,0.0,0.0) + glm::dvec3(ix*d2Array_Offset.x, iy*d2Array_Offset.y,0.0);

//				if(iy % 2 == 1 && ix == 0)
//					continue;

				if(2.0*ix + iy >= 4)
					d3Center_Ring += glm::dvec3(0.0005,-0.0005,0.0);
				...
				{// braze
					for(int index_Angle = 0; index_Angle < vAngle.size(); index_Angle++)
					{
						...
						glm::dvec3 d3Center_Braze = d3Center_Ring + glm::dvec3(0.5*dDiameter_Average*glm::cos(dAngle), 0.5*dDiameter_Average*glm::sin(dAngle), 0.0);
						Canvas.drawRectangle(d3Center_Braze, glm::dvec3(0.0015,dThickness_Ring,dOffset), glm::dvec3(0.0,0.0,dAngle+0.5*_PI));
					}
				}
			}
		}
		...
		{// get canvas voxels and create material points
			...
			newMP->d3_Position = vVoxels[index_Voxel].d3_Position + glm::dvec3(d3_Length_Cell.x, d3_Length_Cell.y, 0.5*d3_Length_Grid.z);
