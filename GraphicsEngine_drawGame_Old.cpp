...
//		gl_Canvas_Texture->bindRenderTarget();
	...
	{// Solid, only containing the material points
		...
		for(int index_GP = 0; index_GP < vGridPoint.size(); index_GP++)
		{
			...
			if(thisGP->b3_Fixed.y == false)
				continue;

			// particle position
			float fSize = 0.002;
			Transformation glTransformation(thisGP->d3_Position, glm::vec3(0.0, 0.0, 0.0), glm::vec3(fSize,0.1*fSize,fSize));
			// particle color
...
//		double d6Strain_Yield[6] = {vMaterialPoint_CPDI[0]->d_YieldStress / vMaterialPoint_CPDI[0]->d_ElasticModulus, 0, 0, 0, 0, 0};
		...
		float fJ2_Minimum = CR.d_J2;
		double d6Strain_Maximum[6] = {20.0*vMaterialPoint_CPDI[0]->d_YieldStress/vMaterialPoint_CPDI[0]->d_ElasticModulus, 0, 0, 0, 0, 0};
