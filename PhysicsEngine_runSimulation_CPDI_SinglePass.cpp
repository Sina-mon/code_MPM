...
// ----------------------------------------------------------------------------
int PhysicsEngine::runSimulation_CPDI_SinglePass(double dTimeIncrement_Total)
{
		...
		#pragma omp parallel
		{
			...
			for(unsigned int index_MP = 0; index_MP < allMaterialPoint_CPDI.size(); index_MP++)
			{
				...
				for(unsigned int index_AGP = 0; index_AGP < thisMP->v_AGP.size(); index_AGP++)
				{
					...
					// external forces
					#pragma omp atomic
						thisAGP->d3_Force.x += dShapeValue*thisMP->d3_Force_External.x;
					#pragma omp atomic
						thisAGP->d3_Force.y += dShapeValue*thisMP->d3_Force_External.y;
					#pragma omp atomic
