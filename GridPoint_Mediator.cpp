...
// ----------------------------------------------------------------------------
void GridPoint_Mediator::findAdjacentGridPoints(glm::dvec3 d3Position)
{
	v_adjacentGridPoints.resize(0);

	glm::ivec3 i3BottomLeftFar = glm::floor(d3Position / d3_Length_Cell);
	glm::ivec3 i3Layers_Start = glm::ivec3(0,0,0);
	glm::ivec3 i3Layers_End = glm::ivec3(2,2,2);

	double dTolerance = 0.0;//1.0e-6;

	if((((double)i3BottomLeftFar.x + 1.0 - d3Position.x / d3_Length_Cell.x)) < dTolerance)
		i3Layers_Start.x = 1;
	if((((double)i3BottomLeftFar.y + 1.0 - d3Position.y / d3_Length_Cell.y)) < dTolerance)
		i3Layers_Start.y = 1;
	if((((double)i3BottomLeftFar.z + 1.0 - d3Position.z / d3_Length_Cell.z)) < dTolerance)
		i3Layers_Start.z = 1;

	if(((d3Position.x / d3_Length_Cell.x) - (double)i3BottomLeftFar.x) < dTolerance)
		i3Layers_End.x = 1;
	if(((d3Position.y / d3_Length_Cell.y) - (double)i3BottomLeftFar.y) < dTolerance)
		i3Layers_End.y = 1;
	if(((d3Position.z / d3_Length_Cell.z) - (double)i3BottomLeftFar.z) < dTolerance)
		i3Layers_End.z = 1;

	int index = 0;

	for(int ix = i3Layers_Start.x; ix < i3Layers_End.x; ix++)
	{
		for(int iy = i3Layers_Start.y; iy < i3Layers_End.y; iy++)
		{
			for(int iz = i3Layers_Start.z; iz < i3Layers_End.z; iz++)
			{
...
// ----------------------------------------------------------------------------
void GridPoint_Mediator::findAdjacentGridPoints_CPDI(MaterialPoint_CPDI_CC *pMP)
...
// ----------------------------------------------------------------------------
void GridPoint_Mediator::calculateBases_Classic(glm::dvec3 d3Position_MP, glm::dvec3 d3Position_GP)
{
	glm::dvec3 d3Distance = d3Position_MP - d3Position_GP;

	glm::dvec3 d3ShapeValue = glm::dvec3(0.0, 0.0, 0.0);
	d3ShapeValue.x = 1.0 - glm::abs(d3Distance.x / d3_Length_Cell.x);
	d3ShapeValue.y = 1.0 - glm::abs(d3Distance.y / d3_Length_Cell.y);
	d3ShapeValue.z = 1.0 - glm::abs(d3Distance.z / d3_Length_Cell.z);
	...
	double dTolerance = 1.0e-12;

////			if(dTolerance < this->d_ShapeValue && this->d_ShapeValue < 1.0-dTolerance)
//			{
////				glm::dvec3 d3Sign = glm::dvec3(0.0,0.0,0.0);
////				if(d3Distance.x > 0.00001)	d3Sign.x = 1.0;
////				if(d3Distance.x < -0.00001)	d3Sign.x = -1.0;
////				if(d3Distance.y > 0.00001)	d3Sign.y = 1.0;
////				if(d3Distance.y < -0.00001)	d3Sign.y = -1.0;
////				if(d3Distance.z > 0.00001)	d3Sign.z = 1.0;
////				if(d3Distance.z < -0.00001)	d3Sign.z = -1.0;
////				this->d3_ShapeGradient.x = -d3ShapeValue.y * d3ShapeValue.z * d3Sign.x / d3_Length_Cell.x;//sina, make sure these are formulated correctly
////				this->d3_ShapeGradient.y = -d3ShapeValue.x * d3ShapeValue.z * d3Sign.y / d3_Length_Cell.y;
////				this->d3_ShapeGradient.z = -d3ShapeValue.x * d3ShapeValue.y * d3Sign.z / d3_Length_Cell.z;
//			if(dTolerance < d3ShapeValue.x && d3ShapeValue.x < 1.0-dTolerance)
...
//			if(dTolerance < d3ShapeValue.y && d3ShapeValue.y < 1.0-dTolerance)
//				this->d3_ShapeGradient.y = -d3ShapeValue.x * d3ShapeValue.z * glm::sign(d3Distance.y) / d3_Length_Cell.y;
//			if(dTolerance < d3ShapeValue.z && d3ShapeValue.z < 1.0-dTolerance)
//				this->d3_ShapeGradient.z = -d3ShapeValue.x * d3ShapeValue.y * glm::sign(d3Distance.z) / d3_Length_Cell.z;
//			}
	if(glm::abs(d3Distance.x / d3_Length_Cell.x) > dTolerance && glm::abs(d3Distance.x / d3_Length_Cell.x) < 1.0-dTolerance)
		this->d3_ShapeGradient.x = -d3ShapeValue.y * d3ShapeValue.z * glm::sign(d3Distance.x) / d3_Length_Cell.x;//sina, make sure these are formulated correctly
	if(glm::abs(d3Distance.y / d3_Length_Cell.y) > dTolerance && glm::abs(d3Distance.y / d3_Length_Cell.y) < 1.0-dTolerance)
		this->d3_ShapeGradient.y = -d3ShapeValue.x * d3ShapeValue.z * glm::sign(d3Distance.y) / d3_Length_Cell.y;
	if(glm::abs(d3Distance.z / d3_Length_Cell.z) > dTolerance && glm::abs(d3Distance.z / d3_Length_Cell.z) < 1.0-dTolerance)
		this->d3_ShapeGradient.z = -d3ShapeValue.x * d3ShapeValue.y * glm::sign(d3Distance.z) / d3_Length_Cell.z;
}

// ----------------------------------------------------------------------------
void GridPoint_Mediator::calculateBases_CPDI(MaterialPoint_CPDI_CC *pMP, glm::dvec3 d3Position_GP)
{
	// shape value ----------------------------------------------------
...
// ----------------------------------------------------------------------------
void GridPoint_Mediator::findNeighborGridPoints(glm::dvec3 d3Position, glm::ivec3 i3Node_Count, glm::dvec3 d3Length_Cell, int iLayers)
