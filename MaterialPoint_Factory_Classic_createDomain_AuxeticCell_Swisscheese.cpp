...
// ----------------------------------------------------------------------------
std::vector<MaterialPoint_BC *> MaterialPoint_Factory_Classic_CC::createDomain_AuxeticCell_Swisscheese(glm::dvec3 d3Center, glm::dvec3 d3Dimension, glm::dvec2 d2Spacing, glm::dvec2 d2Radii, double dOffset)
{
	...
	for(double dx = 0.5*dOffset; dx <= 0.5*d3Dimension.x; dx += dOffset)
	{
		for(double dy = 0.5*dOffset; dy <= 0.5*d3Dimension.y; dy += dOffset)
		{
			for(double dz = dz_Start; dz <= 0.5*d3Dimension.z; dz += dOffset)
			{
				...
				thisMaterialPoint = createMaterialPoint(d3Center + glm::dvec3(+d3Coordinate.x, +d3Coordinate.y, +d3Coordinate.z));
