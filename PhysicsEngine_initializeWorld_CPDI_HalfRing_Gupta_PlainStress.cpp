...
// ----------------------------------------------------------------------------
void PhysicsEngine::initializeWorld_CPDI_HalfRing_Gupta_PlainStress(void)
{
...
//	glm::ivec3 i3_Cells = glm::ivec3(1.0*20, 1.0*30, 2);
	glm::dvec3 d3_Length_Grid = glm::dvec3(0.040, 0.080, 0.002/1.0);
	glm::ivec3 i3_Cells = glm::ivec3(1.0*40, 1.0*80, 2);
...
//			thisGridPoint->b3_Fixed.z = true;
	...
	double dThickness_Ring = 0.00130;// - d_Offset;
	double dDiameter_Average = 0.04870;
...
//	double dLength_Ring = glm::min(0.5*d3_Length_Grid.z, 0.5*(dThickness_Ring/iDivision_Radial + 0.5*_PI*dDiameter_Average/iDivision_Angular));
	...
	glm::dvec3 d3Center_Ring			= glm::dvec3(.0*d3_Length_Cell.x, 0.5*dDiameter_Outer+d3Dimension_Platen_Bottom.y+2.5*d3_Length_Cell.y,0.5*dLength_Ring);
...
//	glm::dvec3 d3Center_Platen_Top		= 0.5*d3Dimension_Platen_Top + glm::dvec3(0.0, d3Center_LoadCell.y+0.5*d3Dimension_LoadCell.y+2.0*d3_Length_Cell.y,0.0);
	glm::dvec3 d3Center_Platen_Top		= 0.5*d3Dimension_Platen_Top + glm::dvec3(0.0, d3Center_Ring.y+0.5*dDiameter_Outer+2.5*d3_Length_Cell.y,0.0);
...
//			thisMP->i_MaterialType = _VONMISESHARDENING;
			...
			thisMP->d_ElasticModulus = 38.0e9;
			thisMP->d_Viscosity = 0.0;
			thisMP->d_PoissonRatio = 0.3;
			thisMP->d_YieldStress = 220.0e6;
...
//			if(thisMP->d3_Position.y < d3Center.y - 0.25*d3Dimension.y)
	...
	std::string sDescription = "";
	{
		...
		sDescription += "-------------------------------------------------------------\n";
		sDescription += "CPDI formulation, Plain Stress, Half Ring Gupta (2005) ------\n";
