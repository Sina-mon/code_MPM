...
// ----------------------------------------------------------------------------
void PhysicsEngine::initializeWorld_Classic_Foam_Ring(void)
{
	MaterialPoint_Factory_Classic_CC	MP_Factory;
	GridPoint_Factory					GP_Factory;
	// ------------------------------------------------------------------------
	// grid points ------------------------------------------------------------
	glm::dvec3 d3_Length_Grid = glm::dvec3(0.060, 0.050, 0.004/2.0);
	glm::ivec3 i3_Cells = glm::ivec3(2.0*60, 2.0*50, 2);
...
//			thisGridPoint->b3_Fixed.x = true;
	...
	glm::dvec3 d3Dimension_Platen_Top		= glm::dvec3(0.8*d3_Length_World.x,1.0*d3_Length_Cell.y,d3_Length_Grid.z);
	glm::dvec3 d3Dimension_Bullet	= glm::dvec3(0.005, 0.006,dOffset_Core);

	glm::dvec3 d3Center_Ring			= glm::dvec3(0.5*d3_Length_Grid.x, 0.5*dDiameter_Outer+3.0*d3_Length_Cell.y, 0.5*dOffset_Core);
	...
	glm::dvec3 d3Center_Platen_Top		= glm::dvec3(0.5*d3_Length_Grid.x, d3Center_Ring.y+0.5*dDiameter_Outer+3.0*d3_Length_Cell.y,0.0);
	glm::dvec3 d3Center_Bullet			= d3Center_Core + glm::dvec3(0.0, 0.65*d3Dimension_Core.y, 0.0);
...
//			thisMP->i_MaterialType = _VONMISESHARDENING;
			...
			thisMP->d_ElasticModulus = 20.0e9;
			thisMP->d_Viscosity = 0.0;
			thisMP->d_PoissonRatio = 0.3;
			thisMP->d_YieldStress = 100.0e6;
...
//					allMaterialPoint.pop_back();
	...
	{// ring ------------------------------------------------------------------ ring
		...
		std::vector<MaterialPoint_BC *> thisMaterialDomain = MP_Factory.createDomain_Tube(d3Center_Ring, glm::dvec3(0.0,0.0,0.0), dRadius_Outer, dRadius_Inner, dOffset_Core, dOffset_Core);
//		std::vector<MaterialPoint_BC *> thisMaterialDomain = MP_Factory.createDomain_Cuboid(d3Center_Core, d3Dimension_Core, dOffset_Core);
		for(unsigned int index_MP = 0; index_MP < thisMaterialDomain.size(); index_MP++)
		{// assign material point initial values
			MaterialPoint_BC *thisMP = thisMaterialDomain[index_MP];
...
//			if(false)
	...
	if(false)
	{// bullet ---------------------------------------------------------------- bullet
		std::vector<MaterialPoint_BC *> thisMaterialDomain = MP_Factory.createDomain_Cuboid(d3Center_Bullet, d3Dimension_Bullet, dOffset_Core);
		for(unsigned int index_MP = 0; index_MP < thisMaterialDomain.size(); index_MP++)
		{// assign material point initial values
			MaterialPoint_BC *thisMP = thisMaterialDomain[index_MP];
			...
			thisMP->d3_Velocity = glm::dvec3(0.0, -700.0, 0.0);
			thisMP->d3_Force_External = thisMP->d_Mass * glm::dvec3(0.0, 0.0, 0.0);
		}
		for(unsigned int index_MP = 0; index_MP < thisMaterialDomain.size(); index_MP++)
		{// send to MP vectors
...
//	d_TimeEnd = 0.5*d3Dimension_Core.y / glm::abs(dPlatenSpeed);
	d_TimeConsole_Interval = 10.0e-5;
	d_TimeEnd = 20.0*d_TimeConsole_Interval;
